def config_handler(request: Request, response: Response) -> None:
    ...
    try:
        data = request.json()
        validated = validate_config_input(data)
        result = process_config(validated)
...
async def fetch_metrics(session: ClientSession, url: str) -> dict:
